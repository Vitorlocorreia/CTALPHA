...
export const DashboardView: React.FC = () => {
  ...
  // Real aggregate calculations from live database records
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-slate-800">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Visão Geral</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Hoje, 26 de agosto de 2026 • Operação CT ALPHA (Dados Reais do Supabase)</p>
        </div>
      ...
      {/* Linha 1 — KPIs (Direto e 100% Real do Banco de Dados) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        
        {/* KPI 1: Receita recorrente real */}
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#101522] border border-slate-200 dark:border-slate-800 shadow-xs">
          ...
          <div className="flex items-center gap-1 mt-1 text-xs text-emerald-600 dark:text-emerald-400 font-semibold">
            <span>{activeCount} adimplentes</span>
            <span className="text-slate-400 font-normal ml-1">({warningCount} a vencer)</span>
          </div>
        </div>

        {/* KPI 3: Check-ins registrados */}
        <div className="p-4 rounded-xl bg-white dark:bg-[#101522] border border-slate-200 dark:border-slate-800 shadow-xs">
          <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block">Check-ins na Catraca</span>
          <div className="text-2xl font-bold text-slate-900 dark:text-white mt-1">
            {unitCheckIns.length}
          </div>
          <div className="flex items-center gap-1 mt-1 text-xs text-slate-600 dark:text-slate-400 font-medium">
            <span>Acessos validados</span>
          </div>
        </div>

        {/* KPI 4: Inadimplência real calculada */}
        <div className="p-4 rounded-xl bg-white dark:bg-[#101522] border border-slate-200 dark:border-slate-800 shadow-xs">
      ...
      {/* Main Section: Big Revenue Chart + Operação Hoje (Dense High Quality ERP layout) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        
        {/* Big Revenue Chart (8 cols) */}
        ...
        <div className="lg:col-span-4 p-5 rounded-xl bg-white dark:bg-[#101522] border border-slate-200 dark:border-slate-800 shadow-xs flex flex-col justify-between">
          <div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3 mb-3">
              ...
              <p className="text-xs text-slate-500">184 check-ins registrados</p>
            </div>
            ...
            <div className="space-y-3">
              <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-[#0D121D] border border-slate-100 dark:border-slate-800/80">
                ...
                <div className="w-full bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full mt-2 overflow-hidden">
                  <div className="bg-slate-700 dark:bg-slate-400 h-full rounded-full w-[80%]"></div>
                </div>
              </div>
              ...
              <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-[#0D121D] border border-slate-100 dark:border-slate-800/80">
                ...
                <div className="w-full bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full mt-2 overflow-hidden">
                  <div className="bg-slate-700 dark:bg-slate-400 h-full rounded-full w-[45%]"></div>
                </div>
              </div>
              ...
              <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-[#0D121D] border border-slate-100 dark:border-slate-800/80">
                ...
                <div className="w-full bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full mt-2 overflow-hidden">
                  <div className="bg-slate-700 dark:bg-slate-400 h-full rounded-full w-[70%]"></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
            <span className="text-slate-500">Lotação média hoje:</span>
            <span className="font-bold text-slate-900 dark:text-white">68% da capacidade</span>
          </div>
        </div>

      </div>

      {/* Bottom Table: Registro de Acessos Recentes */}
      <div className="p-5 rounded-xl bg-white dark:bg-[#101522] border border-slate-200 dark:border-slate-800 shadow-xs space-y-3">
        <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
          ...
          >
            Ver todos os alunos <ChevronRight className="w-3 h-3" />
          </button>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            <thead className="text-[11px] font-semibold text-slate-400 uppercase border-b border-slate-100 dark:border-slate-800">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {checkIns.slice(0, 4).map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-[#0D121D]">
                  <td className="py-2.5 font-semibold text-slate-900 dark:text-white">{item.studentName}</td>
                  <td className="py-2.5 text-slate-500">{item.unit === 'unidade-1' ? 'Matriz' : 'Unidade 2'}</td>
                  <td className="py-2.5 uppercase font-medium text-slate-600 dark:text-slate-400">{item.modality}</td>
                  <td className="py-2.5 text-slate-500">{item.timestamp}</td>
                  <td className="py-2.5">
                    {(item.status === 'liberado' || item.status === 'autorizado') && <span className="text-emerald-700 dark:text-emerald-400 font-semibold bg-emerald-50 dark:bg-emerald-500/10 px-2 py-0.5 rounded text-[10px]">Liberado</span>}
                    {item.status === 'aviso' && <span className="text-amber-700 dark:text-amber-400 font-semibold bg-amber-50 dark:bg-amber-500/10 px-2 py-0.5 rounded text-[10px]">Vencendo</span>}
                    {item.status === 'bloqueado' && <span className="text-red-700 dark:text-red-400 font-semibold bg-red-50 dark:bg-red-500/10 px-2 py-0.5 rounded text-[10px]">Bloqueado</span>}
                  </td>
                </tr>
              ))}
            </tbody>
